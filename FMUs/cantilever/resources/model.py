...
class Model:
    def __init__(self) -> None:
        ...
        #Directories
        #Finding the path of this python script
        current_path=Path(__file__).resolve()
        folders=os.path.split(current_path)[0]
        os.chdir(folders)
        rep="Test-Bench-Digital-Twin\\"
        while rep in repr(os.getcwd()):
            os.chdir("../")
        #Saving the repository folder
        rep_folder=os.getcwd()

        #Setting the ccx.exe path
        ccx_exe_path="./CCX Files/calculix2.19win64/ccx"
        os.chdir(ccx_exe_path)
        self.ccx_exe_path=os.path.join(os.getcwd(),"ccx_219.exe")
        os.chdir(rep_folder)
    ...
    def update_inputs(self):
        ...
        new_uy=random.uniform(self.uy-1,self.uy+1)
        while new_uy<-20 or new_uy>20:
            new_uy=random.uniform(self.uy-1,self.uy+1)
        self.uy=new_uy
        sciformat_uy="{:.3e}".format(self.uy)
        print(f"uy={sciformat_uy} mm")

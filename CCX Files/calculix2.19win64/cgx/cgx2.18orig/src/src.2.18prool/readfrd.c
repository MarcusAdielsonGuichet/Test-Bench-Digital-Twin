...
#define fseeko fseek // prool: no fseeko in windows mingw
...
int readfrd( char *datin, Summen *anz, Nodes **nptr, Elements **eptr, Datasets **lptr, int read_mode )
{
  ...
  printf(" gen lc[%d] ncomps:%d\n",anz->l,lcase[anz->l].ncomps);
          ...
          if(indx<lcase[anz->l].ncomps)
	  {
	    ...
	    {
              free(lcase[anz->l].compName[indx]); lcase[anz->l].compName[indx]=NULL;
              free(lcase[anz->l].icname[indx]); lcase[anz->l].icname[indx]=NULL;
	    }
            else ncomps++;
            indx++;
	  }
